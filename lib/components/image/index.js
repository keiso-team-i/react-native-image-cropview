...
import { useGestureEvent, usePanTranslationFrame, useScaleFrame, } from '../../hooks';
import styles from '../../styles';
import { resize } from '../../utils';
const Image = ({ uri, onImageDone, onImageMove, cropBoxRefs, maxScale = maximumAvailableScale, }, ref) => {
    ...
    const displayTranslation = useRef(new Animated.ValueXY({ x: imageLayout.current.x, y: imageLayout.current.y }));
    const displayTranslationTrack = useRef({
        x: imageLayout.current.x,
        y: imageLayout.current.y,
    });
    // track decay animatations values
    const decayValues = useRef({ x: 0, y: 0, stateX: State.END, stateY: State.END });
    displayTranslation.current.addListener((value) => trackDecayAnimations(value));
    useImperativeHandle(ref, () => ({
        setCropboxLayout: (layout) => {
            cropBoxLayout.current = layout;
        },
        pinchToZoom: ({ focalX, focalY, scale, translateX, translateY, }) => {
        ...
        getCropArea: () => {
            ...
            const y = (cropBoxLayout.current.y - apparentTop) / displayScaleTrack.current;
            const x = (cropBoxLayout.current.x - apparentLeft) / displayScaleTrack.current;
            return {
                x,
                y,
    ...
    const pinchGestureEvent = useGestureEvent((event) => {
        ...
        if (state === State.BEGAN) {
            ...
            const { x, y } = getCenterImage();
            const deltaPinchX = (1 - scaleFrame) * (focalX - displayTranslationTrack.current.x - x);
            const deltaPinchY = (1 - scaleFrame) * (focalY - displayTranslationTrack.current.y - y);
            // set track animated values
            const nextScale = Math.max(displayScaleTrack.current * scaleFrame, minScale - 0.2);
    ...
    const getRecenteredImagePositionValues = () => {
        ...
        const clampedScale = Math.min(maxScale, Math.max(displayScaleTrack.current, minScale));
        ...
        return {
            ...
            apparentRight: recenteredX - widthDiff + imageLayout.current.width * clampedScale,
            apparentBottom: recenteredY - heightDiff + imageLayout.current.height * clampedScale,
        };
    };
    const getApparentY = (currentY) => {
        const apparentHeight = imageLayout.current.height * displayScaleTrack.current;
        const heightDiff = (apparentHeight - imageLayout.current.height) / 2;
        const apparentTop = currentY - heightDiff;
        const apparentBottom = currentY -
            heightDiff +
            imageLayout.current.height * displayScaleTrack.current;
        return { apparentTop, apparentBottom };
    };
    const getApparentX = (currentX) => {
        const apparentWidth = imageLayout.current.width * displayScaleTrack.current;
        const widthDiff = (apparentWidth - imageLayout.current.width) / 2;
        const apparentLeft = currentX - widthDiff;
        const apparentRight = currentX -
            widthDiff +
            imageLayout.current.width * displayScaleTrack.current;
        return { apparentLeft, apparentRight };
    };
    const trackDecayAnimations = (value) => {
        ...
        if (decayValues.current.stateY === State.ACTIVE) {
            ...
            if (apparentTop >= cropBoxLayout.current.y ||
        ...
        if (decayValues.current.stateX === State.ACTIVE) {
            ...
            if (apparentLeft >= cropBoxLayout.current.x ||
    ...
    const decayImage = ({ velocityX, velocityY, }) => {
    ...
    const recenterImage = (axis) => {
        const { scale, x, y, apparentLeft, apparentRight, apparentBottom, apparentTop, } = getRecenteredImagePositionValues();
        const animations = [
            Animated.timing(displayScale.current, {
                toValue: scale,
                ...animationConfig,
        ...
        if (axis.x) {
            animations.push(Animated.timing(displayTranslation.current.x, {
                toValue: x,
                ...animationConfig,
            }));
        }
        if (axis.y) {
            animations.push(Animated.timing(displayTranslation.current.y, {
                toValue: y,
                ...animationConfig,
            }));
        }
    ...
    return (React.createElement(Animated.View, { style: styles.imageContainer },
        React.createElement(PinchGestureHandler, { ...pinchGestureEvent, ref: pinchRef, simultaneousHandlers: [panRef] },
            React.createElement(Animated.View, { style: styles.imageContainer },
                React.createElement(PanGestureHandler, { ...panGestureEvent, avgTouches: true, ref: panRef, simultaneousHandlers: pinchRef, maxPointers: 2, shouldCancelWhenOutside: false, minDist: 6, waitFor: cropBoxRefs },
                    React.createElement(Animated.Image, { source: { uri }, style: {
