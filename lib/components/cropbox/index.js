...
import { ActivityIndicator, Animated, Dimensions, Easing, View, } from 'react-native';
import { PanGestureHandler, State } from 'react-native-gesture-handler';
import { animationConfig, borderWidth, cornerSize } from '../../config';
import { useGestureEvent, usePanTranslationFrame, useSharedValue, useTimeout, } from '../../hooks';
import styles from '../../styles';
import { BoxHandlerTypes, } from '../../types';
import { clamp, getMinCropperSize, resize } from '../../utils';
import Indicators from './indicators';
import Overlay from './overlay';
const CropBox = ({ onCropBoxDone, updateImageBounderies, cropBoxRefs, lockedAspectRatio, rounded, }, ref) => {
    // Using state for the screen sizes to rerender the sizes of the backdrop
    // const [screen, setScreen] = useState(Dimensions.get("screen"));
    // Keep only one active handler at time
    const [activeHandler, setActiveHandler] = useState(BoxHandlerTypes.None);
    ...
    useImperativeHandle(ref, () => ({
        ...
        updateCropperToFitInsideContainer: (containerLayoutMeasures) => {
    ...
    const [panTranslationFrame, resetPanTranslationFrame] = usePanTranslationFrame();
    ...
    const [cropperAnimationsTimer, clearCropperAnimationsTimer, resetCropperAnimationTimer,] = useTimeout(2000);
    const getClmpedValues = ({ movingTop, movingLeft, movingRight, movingBottom, }) => {
        if (!lockedAspectRatio) {
            return {
                leftvalues: {
        ...
        const minRightValue = Math.min(imageLayout.current.right, right.value.current);
        const minBottomValue = Math.min(imageLayout.current.bottom, bottom.value.current);
        var currentWidth = 0;
        var currentHeight = 0;
        if (movingTop) {
        ...
        if (currentWidth === 0 || currentHeight === 0) {
            console.error('Something went wrong with the current width and height clamp calculations. Check getClmpedValues() function');
        }
        // get the new minimum sizes for the cropper
        const resizedWidth = Math.max(Math.max((currentHeight * initialCropperSize.current.width) /
            initialCropperSize.current.height, minCropperBoxSize.current.width), minCropperBoxSize.current.width);
        const resizedHeight = Math.max(Math.max((currentWidth * initialCropperSize.current.height) /
            initialCropperSize.current.width, minCropperBoxSize.current.height), minCropperBoxSize.current.height);
        const minTranslationTop = Math.max(bottom.value.current - resizedHeight, imageLayout.current.top);
        const maxTranslationBottom = Math.min(Math.max(top.value.current + resizedHeight, top.value.current + minCropperBoxSize.current.height), imageLayout.current.bottom);
        const minTranslationLeft = Math.max(right.value.current - resizedWidth, imageLayout.current.left);
        const maxTranslationRight = Math.min(Math.max(left.value.current + resizedWidth, left.value.current + minCropperBoxSize.current.width), imageLayout.current.right);
        return {
            leftvalues: {
    ...
    const scaleRectangleAndFitIntoPosition = () => {
        const { width, height, x, y, scale, focalX, focalY, translateX, translateY, } = getNextBoxBounderies();
        onCropBoxDone({
            width,
            height,
            x,
    ...
    const onDragHandlerStart = () => {
        ...
        stopAllAnimations();
        (_a = indicatorsRef.current) === null || _a === void 0 ? void 0 : _a.showGrid(true);
    };
    const onDragHandlerEnd = () => {
        ...
        cropperAnimationsTimer(() => {
            ...
            (_a = indicatorsRef.current) === null || _a === void 0 ? void 0 : _a.showGrid(false);
        });
    };
     ...
     */
    const topMiddleBar = Animated.subtract(Animated.add(left.animatedValue.current, Animated.divide(Animated.subtract(right.animatedValue.current, left.animatedValue.current), 2)), cornerSize / 2);
    const bottomMiddleBar = Animated.subtract(Animated.add(left.animatedValue.current, Animated.divide(Animated.subtract(right.animatedValue.current, left.animatedValue.current), 2)), cornerSize / 2);
    const leftMiddleBar = Animated.subtract(Animated.add(bottom.animatedValue.current, Animated.divide(Animated.subtract(top.animatedValue.current, bottom.animatedValue.current), 2)), cornerSize / 2);
    const rightMiddleBar = Animated.subtract(Animated.add(bottom.animatedValue.current, Animated.divide(Animated.subtract(top.animatedValue.current, bottom.animatedValue.current), 2)), cornerSize / 2);
    const backdropOpacity = opacityAnimation.current.interpolate({
        inputRange: [0, 1],
        outputRange: [1, 0.6],
    });
    const enabled = (handlerType) => {
        const allHandlers = [
        ...
        return (activeHandler === BoxHandlerTypes.None ||
    ...
    if (!initialCropperSize.current.width && !containerLayout.current.width) {
        return (React.createElement(View, { pointerEvents: 'none', style: styles.cropboxLoaderContainer },
            React.createElement(ActivityIndicator, { size: 'small', color: '#ffffff' })));
    }
    return (React.createElement(React.Fragment, null,
        React.createElement(Indicators, { ref: indicatorsRef, top: top, bottom: bottom, right: right, left: left, containerWidth: containerLayout.current.width, containerHeight: containerLayout.current.height }),
        React.createElement(Overlay, { top: top, bottom: bottom, right: right, left: left, rounded: rounded, containerWidth: containerLayout.current.width, containerHeight: containerLayout.current.height, backdropOpacity: backdropOpacity, minCropperBoxSize: minCropperBoxSize.current }),
        React.createElement(PanGestureHandler, { ...onTopLeftCornerGestureEvent, ref: cropBoxRefs[0], enabled: enabled(BoxHandlerTypes.TopLeft), avgTouches: true, hitSlop: hitSlop, minDist: 2 },
            React.createElement(Animated.View, { style: [
                    ...
                    styles.topLeftCorner,
                    {
                        transform: [
                            {
                                translateX: Animated.subtract(left.animatedValue.current, borderWidth),
                            },
                            {
                                translateY: Animated.subtract(top.animatedValue.current, borderWidth),
                            },
                        ],
                    },
                ] })),
        React.createElement(PanGestureHandler, { ...onTopRightCornerGestureEvent, ref: cropBoxRefs[1], enabled: enabled(BoxHandlerTypes.TopRight), avgTouches: true, hitSlop: hitSlop, minDist: 2 },
            React.createElement(Animated.View, { style: [
                    styles.corner,
                    styles.topRightCorner,
                    {
                        transform: [
                            {
                                translateX: Animated.subtract(right.animatedValue.current, cornerSize - borderWidth),
                            },
                            {
                                translateY: Animated.subtract(top.animatedValue.current, borderWidth),
                            },
        ...
        React.createElement(PanGestureHandler, { ...onBottomLeftCornerGestureEvent, ref: cropBoxRefs[2], enabled: enabled(BoxHandlerTypes.BottomLeft), avgTouches: true, hitSlop: hitSlop, minDist: 2 },
            React.createElement(Animated.View, { style: [
                    styles.corner,
                    styles.bottomLeftCorner,
                    {
                        transform: [
                            ...
                            {
                                translateY: Animated.subtract(bottom.animatedValue.current, cornerSize - borderWidth),
                            },
                        ],
                    },
                ] })),
        React.createElement(PanGestureHandler, { ...onBottomRightCornerGestureEvent, ref: cropBoxRefs[3], enabled: enabled(BoxHandlerTypes.BottomRight), avgTouches: true, hitSlop: hitSlop, minDist: 2 },
            React.createElement(Animated.View, { style: [
                    styles.corner,
        ...
        !lockedAspectRatio && (
        /** @ts-ignore */
        React.createElement(PanGestureHandler, { ...onTopBarGestureEvent, ref: cropBoxRefs[4], enabled: enabled(BoxHandlerTypes.Top), avgTouches: true, hitSlop: hitSlop, minDist: 2 },
            React.createElement(Animated.View, { style: [
                    styles.corner,
                    styles.topLine,
                    {
        ...
        !lockedAspectRatio && (
        /** @ts-ignore */
        React.createElement(PanGestureHandler, { ...onBottomBarGestureEvent, ref: cropBoxRefs[5], enabled: enabled(BoxHandlerTypes.Bottom), avgTouches: true, hitSlop: hitSlop, minDist: 2 },
            React.createElement(Animated.View, { style: [
                    styles.corner,
                    styles.bottomLine,
                    {
        ...
        !lockedAspectRatio && (
        /** @ts-ignore */
        React.createElement(PanGestureHandler, { ...onLeftBarGestureEvent, ref: cropBoxRefs[6], enabled: enabled(BoxHandlerTypes.Left), avgTouches: true, hitSlop: hitSlop, minDist: 2 },
            React.createElement(Animated.View, { style: [
                    styles.corner,
                    styles.leftLine,
                    {
        ...
        !lockedAspectRatio && (
        /** @ts-ignore */
        React.createElement(PanGestureHandler, { ...onRightBarGestureEvent, ref: cropBoxRefs[7], enabled: enabled(BoxHandlerTypes.Right), avgTouches: true, hitSlop: hitSlop, minDist: 2 },
            React.createElement(Animated.View, { style: [
                    styles.corner,
                    styles.rightLine,
                    {
